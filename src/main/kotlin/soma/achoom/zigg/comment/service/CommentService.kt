    ...
    fun unlikeComment(authentication: Authentication, boardId: Long, postId: Long, commentId: Long):CommentResponseDto{
     ...
     fun generateCommentResponse(comment: Comment,user: User): CommentResponseDto{
        return CommentResponseDto(
            ...
            commentMessage = if(comment.isDeleted) "삭제된 메세지입니다." else if (comment.creator.user in user.ignoreUsers) "차단된 유자" else comment.textComment,
            commentCreator = UserResponseDto(
                userId = comment.creator.user?.userId,
                userName = if(comment.isDeleted || comment.creator.user == null) "알수없음" else if (comment.creator.user in user.ignoreUsers) "차단되어 숨김 처리된 메세지입니다." else if (comment.creator.anonymous) comment.creator.anonymousName else comment.creator.user?.name,
            ...
            childComment = comment.replies.map { reply->
                CommentResponseDto(
                    ...
                    commentMessage = if(reply.isDeleted) "삭제된 메세지입니다." else if (reply.creator.user in user.ignoreUsers) "차단된 유저" else reply.textComment,
                    commentCreator = UserResponseDto(
                        userId = reply.creator.user?.userId,
                        userName = if(reply.creator.user == null || reply.isDeleted) "알수없음" else if (reply.creator.user in user.ignoreUsers) "차단되어 숨김 처리된 메세지입니다." else if(reply.creator.anonymous) reply.creator.anonymousName else reply.creator.user?.name,
